...
#include "lv_conf.h"
#include "lv_demos.h" // IWYU pragma: keep
...
static esp_err_t app_lcd_init(void) {
  ...
  ESP_RETURN_ON_ERROR(
      spi_bus_initialize(EXAMPLE_LCD_SPI_NUM, &buscfg, SPI_DMA_CH_AUTO), TAG,
      "SPI init failed");
  ...
  ESP_GOTO_ON_ERROR(
      esp_lcd_new_panel_io_spi((esp_lcd_spi_bus_handle_t)EXAMPLE_LCD_SPI_NUM,
                               &io_config, &lcd_io),
...
static void example_lvgl_touch_cb(lv_indev_drv_t *drv, lv_indev_data_t *data) {
  ...
  bool tp_pressed =
      esp_lcd_touch_get_coordinates(tp, &tp_x, &tp_y, NULL, &tp_cnt, 1);
